...
use crate::backends::conch::control::{self, ConchCreateOptions};
...
fn validate_conch_config(
    ...
    if options.image_name.is_none() && options.snapshot_id.as_deref().unwrap_or_default().is_empty() {
        return Err(OperationBackendBuildError::InvalidConfig {
...
fn build_state(
    validated: &ValidatedConchConfig,
