...
impl OperationPathResolver for ConchPathResolver {
    ...
    async fn resolve_path(
        ...
        let base = match request.base {
            ...
            ResolveBase::HomeDir => self.state.home_dir.as_ref().ok_or_else(|| OperationError::Unsupported {
                message: "home_dir is not configured".to_string(),
            })?,
            ResolveBase::Explicit(path) => return self.state.resolve_backend_path(request.raw_path.as_str(), &path),
        };
        self.state.resolve_backend_path(request.raw_path.as_str(), base)
    }
}
