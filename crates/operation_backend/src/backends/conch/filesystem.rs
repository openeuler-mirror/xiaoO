...
use agent_contracts::backend::{
    ...
    BackendPath, ExportedFile, ExportedFileSource, OperationError, PathKind, PathStat,
};
...
impl OperationFileSystem for ConchFileSystem {
    ...
    async fn temp_path(&self, request: TempPathRequest) -> Result<BackendPath, OperationError> {
        ...
        let output = self.exec.run_shell_script(script.as_str(), None).await?;
        let text = String::from_utf8_lossy(output.stdout.as_slice()).trim().to_string();
        Ok(BackendPath(text))
    }
}
...
impl OperationExport for ConchFileSystem {
    async fn export_file(
        ...
        let content = agent::get_file(&self.state, request.path.0.as_str()).await?;
        ...
        Ok(ExportedFile {
            ...
            media_type: None,
            source: ExportedFileSource::Bytes(content),
        })
    }
}
    ...
    for line in output.lines() {
        if let Some(value) = line.strip_prefix("exists=") {
            ...
            if let Ok(seconds) = value.parse::<u64>() {
                modified_at = Some(std::time::SystemTime::UNIX_EPOCH + std::time::Duration::from_secs(seconds));
            }
        }
    }
    Ok(PathStat {
