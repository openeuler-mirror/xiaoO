...
pub trait LspProvider: Send + Sync {
    ...
    /// Explicitly sync file content into the LSP server.
    ///
    /// Called by executors that read file bytes through the operation backend
    /// (e.g. remote backends where content is not on the host filesystem).
    /// The default no-op is intentional: for the local backend the LSP service
