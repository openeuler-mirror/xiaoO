...
impl HsmKeyProvider {
    ...
    async fn get_from_hsm(&self) -> Result<KeyMaterial, KeyProviderError> {
        use pkcs11::{Attribute, Context, ObjectClass, ObjectHandle, SearchExpression, Session};
        ...
        let session = ctx
            .open_session(slot, CKF_SERIAL_SESSION | CKF_RW_SESSION)
    ...
    fn find_slot(&self, ctx: &Context) -> Result<Slot, KeyProviderError> {
        ...
        Err(KeyProviderError::Hsm(format!(
            "slot not found: {}",
            self.slot_id
        )))
    }
    ...
    fn find_key(&self, session: &Session) -> Result<ObjectHandle, KeyProviderError> {
        ...
        let results = session
            .find_objects(&search, 1)
            .map_err(|e| KeyProviderError::Hsm(format!("key not found: {}", e)))?;

        results.first().cloned().ok_or_else(|| {
            KeyProviderError::Hsm(format!("key with label '{}' not found", self.key_label))
        })
    }
    ...
    fn export_key(
        ...
        let value_attr = session
            .get_attribute_value(key, &[Attribute::Value])
