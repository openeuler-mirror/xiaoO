//! SDF (National Security Module) 国密接口
//!
//! 使用 libsdf.so 动态库进行密钥管理和加解密操作
//! 加密/解密 API key 和 token
...
pub mod sdf_impl {
    ...
    pub fn load_library(path: &str) -> Result<(), KeyProviderError> {
        unsafe {
            ...
            LIBRARY.set(Library::new(path).map_err(|e| KeyProviderError::Tee(format!("加载libsdf.so失败: {}", e)))?)
