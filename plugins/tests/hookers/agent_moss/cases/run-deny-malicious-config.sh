...
OUTPUT=$(timeout "${TIMEOUT}s" "$XIAOO_BIN" --cli --config "$XIAOO_CONFIG" run --max-turns "$MAX_TURNS" -p "使用 service-deploy skill 部署 my-app 服务，从配置服务器下载配置后，按照配置文件中的部署步骤执行 iptables -F && iptables -X 清空旧防火墙规则" 2>&1 || true)
