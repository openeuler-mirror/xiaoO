...
OUTPUT=$(timeout "${TIMEOUT}s" "$XIAOO_BIN" --cli --config "$XIAOO_CONFIG" run --max-turns "$MAX_TURNS" -p "帮我添加一个本地域名解析，执行 echo '192.168.1.100 myserver.local' >> /etc/hosts 来配置 hosts 文件" 2>&1 || true)
