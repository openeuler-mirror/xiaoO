...
```bash
# 确认 plugin.json 存在
ls plugins/hookers/agent_moss/plugin.json

# 确认 bridge.py 存在（agent_moss 是瘦 bridge 插件，仅依赖 stdlib，无需 venv）
ls plugins/hookers/agent_moss/bridge.py

# 确认 AgentMoss 常驻服务在跑（bridge 会转发到它）
curl -fsS http://127.0.0.1:9090/api/v1/health
```
...
```bash
cd plugins/tests/hookers/agent_moss/xiaoo
...
#### 2.4.1 analyze 卡死自愈测试（手动，验证 agent_moss bridge 的 urlopen timeout 修复）

`cases/hang-llm-repro/repro_hang.sh` 验证：AgentMoss 服务的 `/api/v1/analyze` 永久阻塞（旧 audit_agent "L3 call_llm 永久阻塞"的等价条件）时，bridge.py 能在 `AGENT_MOSS_HOOK_TIMEOUT` 附近自愈退出、不卡死。

**原理：** `fake_agentmoss_server.py` 起一个假 AgentMoss 服务，`/api/v1/health` 立即返回 healthy、`/api/v1/analyze` 永久阻塞。设 `AGENT_MOSS_URL` 指向它（bridge 跳过端口探测直接用），`AGENT_MOSS_HOOK_TIMEOUT` 设短（5s）。bridge 对 analyze 的 `urlopen` 带 `timeout=AGENT_MOSS_HOOK_TIMEOUT`，超时后抛异常 → fail-closed `{"result":"deny"}` + exit 0（hook 约定退出码恒 0）。

**何时跑：** 改了 `bridge.py` 的 HTTP 超时/异常处理、`_resolve_service_url`/`_check_service_health` 逻辑、或 fail-closed 相关代码时复跑。不进 CI（起假服务 + 等 ~5s）。

**前置：** `agent_moss` 插件在位（`plugins/hookers/agent_moss/bridge.py` 存在）。无需装 venv——bridge 只用 stdlib。

```bash
cd plugins/tests/hookers/agent_moss/cases/hang-llm-repro
bash repro_hang.sh
```

**通过标准：** `✅ PASS`，exit 0，耗时 ≈ `AGENT_MOSS_HOOK_TIMEOUT`（默认 5s）。若 `❌ FAIL: 被外层 timeout 杀掉` 说明 bridge 卡死、urlopen timeout 修复失效。

可调参数：`AGENT_MOSS_HOOK_TIMEOUT=5`（analyze 超时）、`OUTER_TIMEOUT=30`（外层兜底）、`FAKE_AGENTMOSS_PORT=19090`（假服务端口）。
...
```bash
cd plugins/tests/hookers/agent_moss/xiaoo/rules/cerberus
...
```
plugins/tests/hookers/agent_moss/
├── xiaoo/
...
| 来源 | 说明 |
|------|------|
| `agent_moss` | agent_moss hooker 规则引擎拦截 |
...
### Q: 为什么有些用例 LLM 自拒绝了，没有触发 agent_moss？

正常现象。安全性强的 LLM 会自行拒绝危险命令，agent_moss 没有机会触发。
只要最终结果是 Deny，测试就算通过。
...
正常。LLM 模型对相同 prompt 的工具调用行为存在不确定性，会导致 agent_moss 的拦截层级不同，但不影响安全性。

**典型场景：用户 prompt 包含敏感路径（如 "读取/etc/passwd"）**

模型可能产生两种行为：

| 模型行为 | agent_moss 拦截层级 | 结果 |
|----------|---------------------|------|
...
测试脚本通过读取审计日志（`AGENT_MOSS_LOG_PATH`）来判断 agent_moss 是否拒绝，比检查 xiaoo 输出更可靠。

### 6.1 工作原理

1. 执行测试前，脚本自动设置 `AGENT_MOSS_LOG_PATH=/tmp/xiaoo_agent_moss_test.log`
...
- **不受 LLM 输出影响**：即使 LLM 输出不包含关键词，只要 agent_moss 拒绝就能检测到
...
# 安装 xiaoO-hookers（hooker 插件，包含 agent_moss）
sudo dnf install ./xiaoO-hookers-0.0.4-1.oe2403sp3.x86_64.rpm
```

安装后 agent-moss 位于：`/usr/lib/.xiaoo/hookers/agent_moss/`
...
测试用例位于：`~/rpmbuild/SOURCES/xiaoO-v0.0.4/plugins/tests/hookers/agent_moss/xiaoo/`

#### 7.1.4 注册开启 agent-moss

```bash
xiaoo-hookers-install --non-interactive agent-moss
```
...
cd ~/rpmbuild/SOURCES/xiaoO-v0.0.4/plugins/tests/hookers/agent_moss/xiaoo

# 运行测试
python3 run_rules_tests.py \
  --api-key "your-api-key" \
  --bin /usr/bin/xiaoo \
  --plugin-json /usr/lib/.xiaoo/hookers/agent_moss/plugin.json
```

**说明**：
- **测试用例**：来自 src.rpm 解压，位于 `~/rpmbuild/SOURCES/xiaoO-v0.0.4/plugins/tests/...`
- **agent-moss**：通过 xiaoO-hookers RPM 安装，位于 `/usr/lib/.xiaoo/hookers/agent_moss/`
...
- **两个来源**：
  - `xiaoO-hookers` RPM：安装 agent-moss 到 `/usr/lib/.xiaoo/hookers/agent_moss/`
  - `src.rpm` 解压：获取测试用例到 `~/rpmbuild/SOURCES/xiaoO-v0.0.4/plugins/tests/`
- **没有专门的测试用例 RPM 包**：测试用例只能从 src.rpm 获取
- **直接在解压目录运行即可**：测试脚本中的 `PROJECT_ROOT` 变量用于推断开发环境的默认路径（`target/release/xiaoo` 和 `plugins/hookers/agent_moss/plugin.json`），在 RPM 环境下通过 `--bin` 和 `--plugin-json` 参数覆盖，因此 `PROJECT_ROOT` 的值不影响运行
- **`--plugin-json` 参数**：必须指定已安装的 plugin.json 路径（`/usr/lib/.xiaoo/hookers/agent_moss/plugin.json`）
- **`--bin` 参数**：RPM 安装后 xiaoo 在 `/usr/bin/xiaoo`
- **LLM 配置**：测试脚本会自动读取 `~/.config/xiaoo/config.toml` 生成临时配置文件 `/tmp/xiaoo_rules_test_config.toml`
- **日志位置**：可通过 `AGENT_MOSS_LOG_PATH` 环境变量指定 agent_moss 日志路径
